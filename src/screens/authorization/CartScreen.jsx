...
const CartScreen = () => {
  const { products, selectedAll, totalPrice, total } = useSelector(
    (state) => state.cart
  );
  ...
  const deleteAction = (id, itemName) => {
    Alert.alert(
      "Hold on!",
      "Are you sure, want remove " + itemName + " from cart?",
      [
        {
          text: "Cancel",
          onPress: () => null,
  ...
  const renderItem = ({ item }) => (
    <Animated.View entering={FadeIn.delay(200)}>
      <View
        className={`border rounded-xl p-4 flex flex-row justify-between mx-4 my-2 ${
          item.selected ? "border-blue-600" : "border-gray-400"
        }`}
      >
        <View className="flex flex-row gap-1">
          <CheckBox
            onClick={() => handleOnSelectProduct(item.id)}
        ...
        <View className="flex-grow ml-4 justify-around">
          ...
          <View className="flex flex-row justify-between">
            ...
            <View className="flex items-center flex-row gap-3">
              {item.quantity <= 1 ? (
                <TouchableOpacity
                  className="border-blue-800 rounded-lg border-2"
                  onPress={() => deleteAction(item.id, item.name)}
                >
                ...
                <TouchableOpacity
                  className="border-blue-800 rounded-lg border-2"
                  onPress={() => handleOnDecreaseQuantity(item.id)}
                >
  ...
  return (
    <View className="flex-1">
      ...
      <View className="p-4 flex-row flex-shrink-0 justify-between items-center border border-t-1 bg-white border-t-gray-200 shadow-lg">
        ...
        <View className="flex gap-4 flex-row">
          ...
          <TouchableOpacity
            className={`flex-shrink-0 py-2 px-4 rounded-xl ${
              totalPrice == 0 ? "bg-gray-500" : "bg-[#314ea7]"
            }`}
