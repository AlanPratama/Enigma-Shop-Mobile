...
const ConfirmationScreen = () => {
  ...
  const renderItem = ({ item }) => (
    <Animated.View entering={FadeIn.delay(200)}>
      <View
        className={`border-2 rounded-xl p-2 flex flex-row justify-between mx-4 mt-2 border-gray-400`}
      >
        <View>
          <Image
            source={
              item.image.url
                ? {
                    uri: process?.env?.EXPO_PUBLIC_API_URL_IMG + item.image.url,
                  }
        ...
        <View className="p-4 gap-2">
          ...
          <View className="flex-row justify-between">
            ...
            <Text className="line-through text-gray-500">
              Rp {(totalPrice / 1000 + 30000).toLocaleString("id-ID")}
            </Text>
          </View>
          <View className="flex-row justify-between">
          ...
          <View className="flex-row justify-between border-t border-gray-500 pt-2">
            ...
            <Text className="font-bold">
              Rp{" "}
              {(2000 + totalPrice / 10000 + totalPrice).toLocaleString("id-ID")}
            </Text>
          </View>
        </View>
      </View>
